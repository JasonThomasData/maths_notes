...
% This is the speices that I'm going to press
p_sp = 2;
p_prop = 0.1;
...
for t = 1:floor(Tmax/dt)
    ...
    if t*dt == 40              % Could replace this with a term to measure change. If change small enough, then equilibrium reached.
        N_eq = N;
    elseif t*dt > 40
        dN(p_sp) = dN(p_sp) + p_prop;
    end
...
% We expect the signs to match, because -inv(A^-1) is a press perturbation,
% and the numerical result implements the same perturbation
disp("Sign of numerical simulation result");
disp(sign(N - N_eq));

disp("Sign of -inv(beta) simulation result");
press_pred_analytic = sign(-inv(beta));
disp(press_pred_analytic(:,p_sp));
...
for i = 1:5
    subplot(3,2,i);
    plot(time(1:t), record(i, 1:t),'linewidth',2);
    xlabel('Time');
    ylabel('Abundance');
