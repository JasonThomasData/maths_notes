...
# For the first run, we're only trying to determine insignificant parameters, so we don't need to test predictions
...
summaryInfo = smryMCMC_HD( codaSamples )
show(summaryInfo)
...
# Then the variables: smoking, anaemia, diabetes, platelets and high_blood_pressure don't have a significant effect
# on the model. We have insufficient evidence to reject H_0 that these parameters = 0.

# The variables: age, ejection_fraction, serum_creatinine, time are outside the 95% HDI, so we have
# sufficient evidence to reject H_0 that the parameter = 0.
toKeep <- c("age", "ejection_fraction", "serum_creatinine", "time")

# The variables: creatinine_phosphokinase, serum_sodium, sex are just inside the HDI. 
# We have insufficient evidence to reject H_0 that they are = 0, but they still might be useful
# enough to include in the final model
toKeepMaybe <- c("creatinine_phosphokinase", "serum_sodium", "sex")

# Keep variables from results above
toKeepIndependentVariables <- c(toKeep, toKeepMaybe)
toKeepData <- data[,c(toKeepIndependentVariables, "DEATH_EVENT")]

# This allows to take a random training and test partition
shuffledData <- toKeepData[sample(nrow(toKeepData)),] # random sample
#trainingData <- shuffledData[1:(nrow(toKeepData)-5),]
trainingData <- shuffledData[1:(nrow(toKeepData)-270),]
testData <- shuffledData[(length(toKeepData)-4):length(toKeepData),]

##########################################
# WITH MODEL UNCERTAINTY

x <- as.matrix(trainingData[,toKeepIndependentVariables])
y <- as.vector(trainingData[,"DEATH_EVENT"])
xPred <- as.matrix(testData[, toKeepIndependentVariables])
dataList <- list(
  x = x,
  y = y,
  xPred = xPred,
...
model {
  for ( i in 1:Ntotal ) {
    # In JAGS, ilogit is logistic:
    # Note that the guess is a way to deal with outliers forcing ilogit to output 0
    y[i] ~ dbern( mu[i] )
      mu[i] <- guess*(1/2) + (1.0-guess)*ilogit(zbeta0 + zbeta[1]*zx[i,1] + zbeta[2]*zx[i,2] + zbeta[3]*zx[i,3] + zbeta[4]*zx[i,4] + zbeta[5]*zx[i,5] + zbeta[6]*zx[i,6] + zbeta[7]*zx[i,7])
  }
  # Priors vague on standardized scale:
  zbeta0 ~ dnorm( 0 , 1/2^2 )
  # non-informative run
  ...
  # PREDICTIONS
  for ( k in 1:Npred){
    pred[k] <- ilogit(beta0 + sum(beta[1:7] * xPred[k,1:7]))
  }
}
"
writeLines( modelString , con="TEMPmodel.txt" )

parameters = c( "m", "zbeta0", "zbeta", "beta0", "beta", "guess", "pred", "Rsq")
...
parameterNames = varnames(codaSamples) # get all parameter names
for ( parName in parameterNames[1:18] ) {
  diagMCMC( codaSamples , parName=parName ,
            saveName="out-" , saveType="png" )
}
...
# My attempts at model selection have failed. I tried to use code that I can't
