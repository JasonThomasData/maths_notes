...
y = myData$y
type = as.numeric(as.factor(myData$Type)) # converts character to consecutive integer levels

# First, let's see the data
summary(y)
hist(y)
# Then the data is sort of, kind of normal. So let's try to generate a normal distribution
# That normal distribution will need its own mean and sd. 
# Note that the mean parameter should range in [-\inf, \inf] and the SD should range 
# between [0, \inf]. So, choose normal and gamma to represent those parameters, respectively
# This is two factors of data. We want normal to compare two normal distributions
# SEE FILE Gamma-Normal-Model-Diagram.png - the top two distributions are the posteriors that
# JAGS will generate.
...
# The last four lines of the model, above, are the prior distributions
# There are to be two normal distributions, but we don't know the parameters of
# those. The mean of the normal is distributed as normal, and the variance
